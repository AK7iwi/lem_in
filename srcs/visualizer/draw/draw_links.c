									...
									int *end_x, int *end_y)
{
	...
	// Avoid division by zero for rooms at same position
	// can be deleted 
	if (distance < 0.01f)
	{
		*start_x = x1;
...
void	draw_links(SDL_Renderer *renderer, t_room *room, t_normalize *norm)
{
	...
	while (link)
	{
		...
		if (room < link->room)
		{
			x1 = normalize_x(room->x, norm);
			y1 = normalize_y(room->y, norm);
			x2 = normalize_x(link->room->x, norm);
			y2 = normalize_y(link->room->y, norm);
