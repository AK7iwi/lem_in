...
static	bool	is_existing_coordinates(t_map *map, uint32_t x, uint32_t y)
{
	size_t	i = 0;
...
static	bool	validate_coordinates(t_data *data, uint32_t x, uint32_t y)
{
	...
	if (is_existing_coordinates(data->map, x, y))
	{
...
{
	if (is_existing_room(data->map, name))
	{
		data->err.parsing_errors |= E_ROOM_EXISTS;
		return (1);
	}

	return (validate_coordinates(data, x, y));
}
